...
library(tidyverse)
path <- here::here()
ps <- readRDS(file.path(path, "results", "ps.RDS"))
```
...
```
```{r}
plot_ordination(ps.prop,ords.nmds.bray, color = "Species",shape = "CD", type="samples") + 
  geom_point(size=2.5) + 
  scale_color_manual(values=c( "#DA8D06","#000000","#489BE5", "#F53112","#AE43A7", "#336600")) +
  scale_shape_manual(values = c(19,2))
```
```{r}
plot_ordination(ps.prop,ords.nmds.bray, color = "Phylum", shape = "CD", type="taxa") + 
  geom_point(size=2.5) + 
  scale_shape_manual(values = c(19,2))
```
...
```{r}
p <- plot_ordination(ps.prop_Canine,ords.nmds.bray,shape = "CD",color = "Species") + scale_color_manual(values= "#489BE5") + geom_point(size=2.5) + scale_shape_manual(values = c(19,2))
p
```
```{r}
tb <- as_tibble(p$data) %>%
  mutate(Sample = sample_names(ps.prop_Canine))
tb
to_keep <- tb %>%
  filter(NMDS2 < 1.5) %>%
  {.$Sample}
ps.prop_Canine0 <- prune_samples(to_keep, ps.prop_Canine)
ords.nmds.bray0 <- ordinate(ps.prop_Canine0, method = "NMDS",distance = "bray")
```
```{r}
p <- plot_ordination(ps.prop_Canine0, ords.nmds.bray0, shape = "CD", color = "Ab") + geom_point(size=2.5) + scale_shape_manual(values = c(19,2))
p
```
